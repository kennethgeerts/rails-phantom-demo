...
joint.dia.CommandManager = Backbone.Model.extend({
    ...
    addCommand: function(cmdName, cell, graph, options) {
        ...
        var isGraphCommand = (cell instanceof joint.dia.Graph);
    ...
    revertCommand: function(command, opt) {
        ...
        if (_.isArray(command)) {
            batchCommand = _.sortBy(command, this.constructor.batchCommandComparator);
        } else {
    ...
    batchCommandComparator: function(command, index, commands) {

        // Note that `filterBatchCommand` method makes sure there is no `add` and `remove`
        // command in the same batch for the same element.

        // Here we swapping only commands related to the same element, where `change` command
