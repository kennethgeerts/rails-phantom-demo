...
joint.ui.PaperScroller = joint.mvc.View.extend({
    ...
    zoom: function(value, opt) {
        ...
        var center = this.clientToLocalPoint(this.el.clientWidth / 2, this.el.clientHeight / 2);
        var sx = value;
        var sy = value;
        var ox;
        var oy;
    ...
    transitionToPoint: function(x, y, opt) {
        ...
        var localPoint = g.Point(x, y);
        var localCenter = this.clientToLocalPoint(this.el.clientWidth / 2, this.el.clientHeight / 2);
        ...
        this.$background
            ...
            .on(this.transitionEventName, _.bind(function(evt) {
                // Clean-up and set the actual scale and scroll on the paperScroller.
                var paperScroller = this.paperScroller
                    .removeTransition()
                    .zoom(this.scale, { absolute: true })
                    .center(this.x, this.y);
                // Trigger a callback
                var onTransitionEnd = this.onTransitionEnd;
                if (_.isFunction(onTransitionEnd)) {
    ...
    transitionToRect: function(rect, opt) {
        ...
        var center = (opt.center) ? g.Point(opt.center) : rect.center();

        var clientWidth = this.el.clientWidth * visibility;
        var clientHeight = this.el.clientHeight * visibility;
        var clientRect = g.Rect({
        ...
        return this.transitionToPoint(center, _.defaults({ scale: scale }, opt));
    },
